...
是否进入真 ASR/TTS 仍以 [VALIDATION.md](VALIDATION.md) 的行为指标为准，**另批**。阶段 1 稳定合并节点：`v0.1-learning-agent`（开单能力 + 记忆学习 + Notice + Demo）。Sprint 7 进入 Adaptive Memory（确认后的偏好修正），不是语音。
