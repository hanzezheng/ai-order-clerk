...
是否进入真 ASR/TTS 仍以 [VALIDATION.md](VALIDATION.md) 的行为指标为准，**另批**。阶段 1 稳定合并节点：`v0.1-learning-agent`。Sprint 7 Adaptive Memory。Sprint 8A 客户冷启动。Sprint 9A Workbench。Sprint 10A Repository Port 收口（同一套 Port，InMemory 仍是默认测试实现）。不是语音，不接 ERP。
