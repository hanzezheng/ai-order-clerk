...
是否进入真 ASR/TTS 仍以 [VALIDATION.md](VALIDATION.md) 的行为指标为准，**另批**。阶段 1 稳定合并节点：`v0.1-learning-agent`。v0.2-persistent-agent：Port + PostgreSQL 可重启。Sprint 11：Durable Outbox。V0.3A：LLM 默认语言入口。V0.3B：商品理解（规格属性过滤；不建 SKU、不接 ERP）。
