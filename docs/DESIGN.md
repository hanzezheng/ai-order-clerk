...
Fake 测 Runtime；live 测模型是否抽对 SpeechAct。规则兜底成功不算模型成功。Prompt 当前 pin `parser.v4`。Schema 前只做机械归一：根数组包成 `acts`；已知语言槽抬入 `slots`；封闭 type 同义词（`add_item`→`add_line` 等，不映射 confirm）；原文可核对的槽位修补（`mention`→`product_mention`、无货名加数量→`set_qty`、光杆「那个X」→`unknown`、「两个」→`uom=个`）。`sku_id` / `customer_id` 等业务字段仍 `extra=forbid`。禁止塞 Catalog。至少一个模型达到 L0 全绿、stall_oral ≥90% 且无兜底、G1/G2 真 Parser 三轮快照一致，才可宣传 LLM 成功路径。
