...
### 14.26 开单员 V1 Pilot 观察

目标：建立档口试用反馈。不是新功能。

每日记用量与信任；失败记原话并分类（数据 / 设计 / Runtime）。结束只选继续 V1.1 或调整产品流程。成功 = 少记单。
...
### 14.28 开单员 V1 Pilot 数据访问

目标：明确企业事实如何进入 Runtime。不是 CRM，不是 ERP 替代。

ERP 提供事实。Read Adapter 读取和投影。Runtime 只消费投影。ERP 不得控制 Resolver / Confirm / Policy。读取失败不猜。
...
### 14.31 开单员 V1 Pilot 执行手册

目标：第一个真实档口当天怎么跑。不是新功能。

开门到收摊只看老板是否主动用。陪跑可提醒不可代开。必须停则改手写。结束只选扩大、观察或暂停。根据习惯，不根据功能数量。
