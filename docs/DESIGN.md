...
### 14.21 农批 AI 销售开单员 V1（产品规格）

目标：把开单员做成老板一天用得上的岗位。不是新 Employee，不改 Freeze。

DESIGN §14 开头的「V1」是 POC 开单壳。商业员工 V1 覆盖开门 → 接单 → 修改 → 确认 → 查看今日订单 → 收摊：AI 主责接/改/定；今日开单本用眼睛看；货与钱继续人工。

细则 [V1_SALES_CLERK.md](V1_SALES_CLERK.md)、[ADR-026](ADR/ADR-026-v1-sales-clerk-product.md)。

禁止：改 Parser / Policy / Confirm Gate / OrderService / Memory；库存 / 支付 / 财务；submit SO；查账口令；第二员工；ERP 驱动 `confirm_gate`。

### 14.22 开单员 V1 用户旅程（产品化）

目标：把 V1 规格写成老板一天愿意用的工作流。不是新 Agent，不改 Freeze。

主界面是今日开单本。嘴巴只写当前这一张。确认 = 生意事实成立，不是付款 / 发货 / 财务完成。

细则 [V1_SALES_CLERK_USER_JOURNEY.md](V1_SALES_CLERK_USER_JOURNEY.md)、[ADR-027](ADR/ADR-027-v1-user-journey-current-order.md)。

禁止：按客户名自动跳单、查账口令、确认后改单、收款 / 库存 / 财务进开单本。

### 14.23 今日开单本原型（V1 Workbench）

目标：把今日开单本做成老板每天打开的工作台。不是 ERP 前端，不改 Confirm Gate。

第一页：日期、当前订单、待确认、已确认、入账、异常。当前单五态是呈现。Voice First：按住 → 草稿 → 复述。
...
### 14.25 开单员 V1 Pilot 检查

目标：验证真实使用是否可靠。不加功能，不改闸门。

可以进入有监督的档口 Pilot；不能宣称无人值守。成功看少手写、少重复确认、少翻聊天。

细则 [V1_SALES_CLERK_PILOT_CHECKLIST.md](V1_SALES_CLERK_PILOT_CHECKLIST.md)、[ADR-030](ADR/ADR-030-v1-pilot-hardening.md)。

禁止：改 Parser / Confirm Gate；为顺滑而猜同名；库存 / 支付 / 财务；第二员工。
